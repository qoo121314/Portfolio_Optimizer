...
class Portfolio():
    ...
    def __init__(self, data):
        ...
        self.portfolio_cumulative_return = np.exp(
            (self.daily_return * self.weights).sum(axis=1).cumsum())
    ...
    def Return_Plot(self, backtest=None):
        ...
        Portfolio_Return = (self.daily_return *
                            self.weights).sum(axis=1).cumsum()

        if backtest is not None:
            Portfolio_Return = self.backtest_table.cumsum()

        Cum_Trade_Percent_Return = np.exp(Portfolio_Return)
        MDD_Series = (Cum_Trade_Percent_Return.cummax() -
                      Cum_Trade_Percent_Return)/Cum_Trade_Percent_Return.cummax()
        High_index = Cum_Trade_Percent_Return[Cum_Trade_Percent_Return.cummax(
        ) == Cum_Trade_Percent_Return].index

        fig, ax = plt.subplots(2, 1, figsize=[15, 6], gridspec_kw={
                               'height_ratios': [3, 1]})
        sns.set_style('darkgrid')

        sns.lineplot(x=Cum_Trade_Percent_Return.index,
                     y=Cum_Trade_Percent_Return, ax=ax[0], color='0.38')
        sns.scatterplot(
            x=High_index, y=Cum_Trade_Percent_Return.loc[High_index], color='#02ff0f', ax=ax[0])
        ...
        ax[1].set_yticklabels(['{:,.1%}'.format(y)
    ...
    def Get_Sharpe_Ratio(self, begin=None, end=None, weight=None, rf=0.05, backtest=None):
        ...
        Sharpe_Ratio = (Average_Return.loc[start_day: end_day].mean(
        ) - (rf / 252)) * 252 / (Average_Return.loc[start_day: end_day].std() * np.sqrt(252))
    ...
    def Get_Sotino_Ratio(self, begin=None, end=None, weight=None, rf=0.05, threshold=None, backtest=None):
        ...
        DownTable = Average_Return[Average_Return < threshold].apply(
            lambda x: (x - threshold) ** 2)
        DownRisk = np.sqrt(DownTable.sum() / Average_Return.count())
        Sotino_Ratio = (Average_Return.mean() - (rf / 252)) * \
            252 / (DownRisk * np.sqrt(252))
    ...
    def Get_MDD(self, begin=None, end=None, backtest=None):
        ...
        Portfolio_Return = (
            self.daily_return * self.weights).sum(axis=1).loc[start_day:end_day].cumsum()
    ...
    def Summary(self, begin=None, end=None):
        ...
        print('From {}  to {}, {} days.'.format(str(start_day.date()),
                                                str(end_day.date()), (end_day-start_day).days))
        print('-'*60)
        ...
        print('-'*60)
        print('Average Return : {:>55.3f}'.format(
            self.Get_Average_Return(begin=start_day, end=end_day)))
        print('Average Standard Deviation : {:>31.3f}'.format(
            self.Get_Average_SD(begin=start_day, end=end_day)))
        print('Sharpe Ratio : {:>60.3f}'.format(
            self.Get_Sharpe_Ratio(begin=start_day, end=end_day)))
        print('Sotino Ratio : {:>61.3f}'.format(
            self.Get_Sotino_Ratio(begin=start_day, end=end_day)))
        print('Maximum Drop Down : {:>42.3f}'.format(
            self.Get_MDD(begin=start_day, end=end_day)))
        print('-'*60)
    ...
    def __sharpe_target(self, weights):
        ...
        Sharpe_Ratio = (Average_Return.loc[self.__opt_begin: self.__opt_end].mean(
        ) - (self.__rf / 252)) * 252 / (Average_Return.loc[self.__opt_begin: self.__opt_end].std() * np.sqrt(252))
    ...
    def __mdd_target(self, weights):
        ...
        Portfolio_Return = (
            self.daily_return * weights).sum(axis=1).loc[self.__opt_begin: self.__opt_end].cumsum()
    ...
    def __std_target(self, weights):
        ...
        temp = temp.sum(axis=1)
        Average_Return = temp.iloc[1:]
        Standard_Deviation = Average_Return.loc[self.__opt_begin: self.__opt_end].std(
        ) * np.sqrt(252)
    ...
    def Get_Best_Portfolio(self, hedge=None, bothside=None, method='sharpe', hedge_con=True):
        ...
        if hedge == True:
            ...
            if hedge_con == True:
                cons = ({'type': 'eq', 'fun': self.__check_sum_hedge},
                        {'type': 'eq', 'fun': self.__check_sum_hedge2})
            else:
                cons = ({'type': 'eq', 'fun': self.__check_sum_hedge})
        ...
        if method == 'sharpe':
            opt_results = minimize(
                self.__sharpe_target, init_guess, method='SLSQP', bounds=bounds, constraints=cons)
            return -opt_results['fun'], list(opt_results['x'])

        elif method == 'mdd':
            opt_results = minimize(
                self.__mdd_target, init_guess, method='SLSQP', bounds=bounds, constraints=cons)
            return opt_results['fun'], list(opt_results['x'])

        elif method == 'std':
            opt_results = minimize(
                self.__std_target, init_guess, method='SLSQP', bounds=bounds, constraints=cons)
            return opt_results['fun'], list(opt_results['x'])

    def set_weights(self, weights=None):
        if weights == None:
            self.weights = np.ones(
                len(self.price_data.columns)) / len(self.price_data.columns)
        else:
    ...
    def Rolling_test(self, begin=None, end=None, freq='1Y', show=False, method='sharpe', hedge=None, cul_info=False, hedge_con=True):
        ...
        if cul_info == True:
            for i in range(len(timelist)):
                ...
                self.set_weights(self.Get_Best_Portfolio(
                    method=method, hedge=hedge, hedge_con=hedge_con)[1])

                if show == True:
                    self.Summary(begin=timelist[i+1][0], end=timelist[i+1][1])

                res = self.Get_Return(
                    begin=timelist[i+1][0], end=timelist[i+1][1])
                return_list = return_list.append(res)

                return_list = return_list[~return_list.index.duplicated(
                    keep='first')]
    ...
    def Backtest_Summary(self):
        ...
        print('-'*60)
        print('Average Return : {:>55.3f}'.format(
            self.Get_Average_Return(begin=start_day, end=end_day, backtest=True)))
        print('Average Standard Deviation : {:>31.3f}'.format(
            self.Get_Average_SD(begin=start_day, end=end_day, backtest=True)))
        print('Sharpe Ratio : {:>60.3f}'.format(
            self.Get_Sharpe_Ratio(begin=start_day, end=end_day, backtest=True)))
        print('Sotino Ratio : {:>61.3f}'.format(
            self.Get_Sotino_Ratio(begin=start_day, end=end_day, backtest=True)))
        print('Maximum Drop Down : {:>42.3f}'.format(
            self.Get_MDD(begin=start_day, end=end_day, backtest=True)))
        print('-'*60)
    ...
    def Plot_Effcient_Frontier(self):
        ...
        plt.scatter(std_arr, mean_arr, c=sharpe_arr,
